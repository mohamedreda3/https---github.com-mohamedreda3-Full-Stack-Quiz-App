...
<body>
    ...
    <script>
        document.querySelector('form button').onclick = (e) => {
            ...
            fetchData('./functions/teacherauthuntication.php', detailsForm).then(res => res.text()).then(data => {
                if (data != 'Added') {
                    errorMassege(data, 'red');
                } else {
                    errorMassege('Added', 'green');
                    setTimeout(() => location.href = 'adminpanal.php', 1500)
                }
            })
        }
        // teacherauthuntication
    </script>
</body>
